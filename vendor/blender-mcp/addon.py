...
class BlenderMCPServer:
    ...
    def create_rodin_job_main_site(
        ...
        try:
            ...
            files = [
                *[("images", (f"{i:04d}{img_suffix}", b64.b64decode(img))) for i, (img_suffix, img) in enumerate(images)],
    ...
    def get_hunyuan3d_status(self):
        ...
        if enabled:
            match hunyuan3d_mode:
                ...
                case "REPLICATE":
                    if not bpy.context.scene.blendermcp_hunyuan3d_replicate_token:
                        return {
                            "enabled": False,
                            "mode": hunyuan3d_mode,
                            "message": """Hunyuan3D integration is currently enabled, but Replicate API token is not set. To enable it:
                                1. In the 3D Viewport, find the BlenderMCP panel in the sidebar (press N if hidden)
                                2. Keep the 'Use Tencent Hunyuan 3D model generation' checkbox checked
                                3. Set mode to 'Replicate' and fill in your Replicate API token
                                4. Restart the connection to Claude"""
                        }
                case "FAL_AI":
                    if not bpy.context.scene.blendermcp_hunyuan3d_fal_key:
                        return {
                            "enabled": False,
                            "mode": hunyuan3d_mode,
                            "message": """Hunyuan3D integration is currently enabled, but fal.ai API key is not set. To enable it:
                                1. In the 3D Viewport, find the BlenderMCP panel in the sidebar (press N if hidden)
                                2. Keep the 'Use Tencent Hunyuan 3D model generation' checkbox checked
                                3. Set mode to 'fal.ai' and fill in your fal.ai API key
                                4. Restart the connection to Claude"""
                        }
                case _:
                    return {
    ...
    def create_hunyuan_job(self, *args, **kwargs):
        match bpy.context.scene.blendermcp_hunyuan3d_mode:
            ...
            case "REPLICATE":
                return self.create_hunyuan_job_replicate(*args, **kwargs)
            case "FAL_AI":
                return self.create_hunyuan_job_fal_ai(*args, **kwargs)
            case _:
    ...
    def create_hunyuan_job_replicate(
        self,
        text_prompt: str = None,
        image: str = None
    ):
        """Create a Hunyuan3D 3.1 prediction on Replicate."""
        try:
            token = bpy.context.scene.blendermcp_hunyuan3d_replicate_token
            face_count = bpy.context.scene.blendermcp_hunyuan3d_face_count
            texture = bpy.context.scene.blendermcp_hunyuan3d_texture

            if not token:
                return {"error": "Replicate API token is not set"}
            if not text_prompt and not image:
                return {"error": "Prompt or Image is required"}
            ...
            # generate_type: "Normal" = textured, "Geometry" = mesh only
            gen_type = "Normal" if texture else "Geometry"

            if text_prompt and not image:
                input_data["prompt"] = text_prompt
                input_data["generate_type"] = gen_type
            elif image:
                # Convert local file path to data URI
                if not re.match(r'^https?://', image, re.IGNORECASE):
                    try:
                        with open(image, "rb") as f:
                            image_bytes = f.read()
            ...
            response = requests.post(
                "https://api.replicate.com/v1/models/tencent/hunyuan-3d-3.1/predictions",
                headers=headers,
            ...
            if response.status_code in (200, 201):
                ...
                return {
                    ...
                    "message": "Replicate prediction created. Use poll_hunyuan_job_status with the prediction_id to check progress.",
                }
            return {
                "error": f"Replicate API request failed ({response.status_code}): {response.text}"
            }
        except Exception as e:
            return {"error": str(e)}

    def create_hunyuan_job_fal_ai(
        self,
        text_prompt: str = None,
        image: str = None
    ):
        """Create a Hunyuan3D v3 job on fal.ai (async queue)."""
        try:
            fal_key = bpy.context.scene.blendermcp_hunyuan3d_fal_key
            face_count = bpy.context.scene.blendermcp_hunyuan3d_face_count
            texture = bpy.context.scene.blendermcp_hunyuan3d_texture

            if not fal_key:
                return {"error": "fal.ai API key is not set"}
            if not image and not text_prompt:
                return {"error": "Image or text prompt is required"}

            input_data = {
                "face_count": face_count,
                "enable_pbr": texture,
                "generate_type": "Normal" if texture else "Geometry",
            }

            if image:
                # fal.ai requires a URL — convert local files to data URI
                if not re.match(r'^https?://', image, re.IGNORECASE):
                    try:
                        with open(image, "rb") as f:
                            image_bytes = f.read()
                        ...
                        return {"error": f"Image encoding failed: {str(e)}"}
                input_data["input_image_url"] = image
            ...
            if response.status_code in (200, 201):
                return {
                    ...
                    "message": "fal.ai job submitted. Use poll_hunyuan_job_status with the request_id to check progress.",
                }
            return {
                "error": f"fal.ai API request failed ({response.status_code}): {json.dumps(data)}"
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    def poll_hunyuan_job_status(self, *args, **kwargs):
        ...
        if mode == "REPLICATE":
            return self.poll_hunyuan_job_status_replicate(*args, **kwargs)
        if mode == "FAL_AI":
            return self.poll_hunyuan_job_status_fal_ai(*args, **kwargs)
    ...
    def poll_hunyuan_job_status_replicate(self, job_id: str = None, prediction_id: str = None):
        """Poll a Replicate prediction for completion status."""
        pid = prediction_id or job_id
        if not pid:
        ...
        try:
            ...
            if response.status_code != 200:
                return {"error": f"Replicate API request failed ({response.status_code}): {response.text}"}

            result = response.json()
            replicate_status = result.get("status", "unknown")
            ...
            mapped_status = status_map.get(replicate_status, replicate_status.upper())

            poll_result = {"status": mapped_status}

            if mapped_status == "DONE":
                output = result.get("output")
                if output:
                    # Output is the GLB URL directly (string) or first element of list
                    glb_url = output if isinstance(output, str) else output[0] if isinstance(output, list) else None
                    poll_result["glb_url"] = glb_url
                    poll_result["ResultFile3Ds"] = glb_url  # compat key for import_generated_asset_hunyuan

            if mapped_status == "FAILED":
                poll_result["error"] = result.get("error", "Unknown error")

            return poll_result
        except Exception as e:
            return {"error": str(e)}

    def poll_hunyuan_job_status_fal_ai(self, job_id: str = None, request_id: str = None):
        """Poll a fal.ai Hunyuan3D queue job for completion."""
        rid = request_id or job_id
        if not rid:
        ...
        try:
            ...
            response = requests.get(
                f"https://queue.fal.run/fal-ai/hunyuan3d-v3/requests/{rid}/status",
            ...
            mapped = status_map.get(fal_status, fal_status)

            result = {"status": mapped, "fal_status": fal_status}

            if mapped == "DONE":
                # Fetch the actual result to get the GLB URL
                result_response = requests.get(
                    f"https://queue.fal.run/fal-ai/hunyuan3d-v3/requests/{rid}",
                    headers={"Authorization": f"Key {fal_key}"},
                )
                result_data = result_response.json()
                glb_info = result_data.get("model_glb", {})
                glb_url = glb_info.get("url") if isinstance(glb_info, dict) else None
                if glb_url:
                    result["glb_url"] = glb_url
                    result["ResultFile3Ds"] = glb_url
                # Also capture model_urls if available
    ...
    def import_generated_asset_hunyuan(self, *args, **kwargs):
        mode = bpy.context.scene.blendermcp_hunyuan3d_mode
        if mode == "REPLICATE":
            return self.import_generated_asset_hunyuan_replicate(*args, **kwargs)
        if mode == "FAL_AI":
            return self.import_generated_asset_hunyuan_replicate(*args, **kwargs)  # same GLB import logic
    ...
    def import_generated_asset_hunyuan_replicate(self, name: str, zip_file_url: str = None, glb_url: str = None):
        """Import a GLB file from a Replicate prediction output URL."""
        url = glb_url or zip_file_url  # accept either parameter name
        if not url:
            return {"error": "GLB URL is required (pass as glb_url or zip_file_url)"}

        if not re.match(r'^https?://', url, re.IGNORECASE):
            return {"error": "Invalid URL format. Must start with http:// or https://"}

        try:
            # Download GLB to temp file
...
class BLENDERMCP_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene.blendermcp_use_hunyuan3d:
            ...
            if scene.blendermcp_hunyuan3d_mode == 'REPLICATE':
                layout.prop(scene, "blendermcp_hunyuan3d_replicate_token", text="API Token")
                layout.prop(scene, "blendermcp_hunyuan3d_face_count", text="Face Count")
                layout.prop(scene, "blendermcp_hunyuan3d_texture", text="Enable PBR Textures")
            if scene.blendermcp_hunyuan3d_mode == 'FAL_AI':
                layout.prop(scene, "blendermcp_hunyuan3d_fal_key", text="API Key")
                layout.prop(scene, "blendermcp_hunyuan3d_face_count", text="Face Count")
                layout.prop(scene, "blendermcp_hunyuan3d_texture", text="Enable PBR Textures")
...
def register():
    ...
    bpy.types.Scene.blendermcp_hunyuan3d_mode = bpy.props.EnumProperty(
        ...
        items=[
            ...
            ("REPLICATE", "Replicate", "Replicate (tencent/hunyuan-3d-3.1)"),
            ("FAL_AI", "fal.ai", "fal.ai (fal-ai/hunyuan3d-v3)"),
        ],
        default="LOCAL_API"
    )
    ...
    bpy.types.Scene.blendermcp_hunyuan3d_replicate_token = bpy.props.StringProperty(
        name="Replicate API Token",
        subtype="PASSWORD",
        description="API token from replicate.com",
        default=""
    )

    bpy.types.Scene.blendermcp_hunyuan3d_fal_key = bpy.props.StringProperty(
        name="fal.ai API Key",
        subtype="PASSWORD",
        description="API key from fal.ai",
        default=""
    )

    bpy.types.Scene.blendermcp_hunyuan3d_face_count = bpy.props.IntProperty(
        name="Face Count",
...
def unregister():
    ...
    del bpy.types.Scene.blendermcp_hunyuan3d_replicate_token
    del bpy.types.Scene.blendermcp_hunyuan3d_fal_key
    del bpy.types.Scene.blendermcp_hunyuan3d_face_count
